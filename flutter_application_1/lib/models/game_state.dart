...
class PlayerState {
  ...
  // Pending state applied when next card is played
...
class GameState {
  ...
  void switchTurn() {
    // Only increment round after Player B's turn ends (full round complete)
    if (currentTurn == Player.playerB) roundNumber++;
    currentTurn =
        currentTurn == Player.playerA ? Player.playerB : Player.playerA;
    phase = GamePhase.buildPhase;
  }
}
